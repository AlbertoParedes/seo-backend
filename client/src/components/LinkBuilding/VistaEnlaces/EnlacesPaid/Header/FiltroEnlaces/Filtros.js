...
import { setVistaLinkBuilding, setFiltrosEnlacesPaidListaLinkbuilding,setFechaEnlaces,  setFiltrosFreePaid } from '../../../../../../redux/actions';
...
class Filtros extends Component{
  ...
  newEnlace = () => {
    ...
    if(this.props.fecha!==functions.getTodayDate()){
      console.log('No puedes crear enlaces en meses anteriores');
      return false
    }
    ...
    if(total>0){
      ...
      multiPath[`Servicios/Linkbuilding/Paid/Enlaces/clientes/${this.props.cliente_seleccionado.id_cliente}/mensualidades/${this.state.fecha}/enlaces/${key}`] = {
        id_empleado:this.props.empleado.id_empleado,id_enlace:key
      }
      db.update(multiPath)
      .then(()=>{ console.log('OK'); })
      .catch( err => console.log(err));
    }else{
      console.log('No tienes suficiente presupuesto');
    }
