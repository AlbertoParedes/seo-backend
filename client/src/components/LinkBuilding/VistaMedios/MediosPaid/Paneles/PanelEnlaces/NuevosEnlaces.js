...
class NuevosEnlaces extends Component {
  ...
  gurdarEnlaces = () => {
    ...
    if(this.state.n_enlaces.toString().includes('.') || this.state.n_enlaces.toString().includes(',') || this.state.n_enlaces.toString()==='' || (+this.state.n_enlaces)<=0){
      console.log('El numero de enlaces es incorrecto');
      return false;
    }else if(this.state.precio_iva===''){
      console.log('Precio de compra no válido');
      return false;
    }else if((+this.state.precio_iva)<=0){
      console.log('Precio de compra no válido');
      return false;
    }else if(!this.state.id_plataforma){
      console.log('Selecciona un plataforma');
      return false;
    }
  ...
  render() {
    ...
    if(this.state.precio_unidad!=='igualdad'){
      ...
      console.log('suma_enlaces',suma_enlaces);
    }

    var num_precio_iva = false, num_precio_sin_iva= false;
    if(this.state.precio_iva!==''){
      ...
      if(num_precio_iva.toString().includes('.') || num_precio_iva.toString().includes(',')){
        var decimales = num_precio_iva.toString().substring(num_precio_iva.toString().indexOf('.'), num_precio_iva.toString().length )
    ...
    if(this.state.precio_sin_iva!==''){
      ...
      if( num_precio_sin_iva.toString().includes('.') || num_precio_sin_iva.toString().includes(',') ){
        var decimales = num_precio_sin_iva.toString().substring(num_precio_sin_iva.toString().indexOf('.'), num_precio_sin_iva.toString().length )
        if(decimales.length>2){
...
export default connect(mapStateToProps)(NuevosEnlaces);
