...
class InformacionMedio extends Component {
  ...
  saveData = () => {
    ...
    var multiPath = {};
    multiPath[`Servicios/Linkbuilding/Paid/Medios/medios/${this.props.id_medio}/web`]=this.state.web.trim();
    multiPath[`Servicios/Linkbuilding/Paid/Medios/medios/${this.props.id_medio}/dr`]=this.state.dr.trim()!==''?(+this.state.dr):null
    multiPath[`Servicios/Linkbuilding/Paid/Medios/medios/${this.props.id_medio}/ur`]=this.state.ur.trim()!==''?(+this.state.ur):null

    var activo = false, eliminado=false;
    if(this.state.status==='Eliminado'){
      multiPath[`Servicios/Linkbuilding/Paid/Medios/medios/${this.props.id_medio}/eliminado`]=true
    }else if(this.state.status==='Activado'){
      multiPath[`Servicios/Linkbuilding/Paid/Medios/medios/${this.props.id_medio}/activo`]=true
      multiPath[`Servicios/Linkbuilding/Paid/Medios/medios/${this.props.id_medio}/eliminado`]=false
    }else if(this.state.status==='Desactivado'){
      multiPath[`Servicios/Linkbuilding/Paid/Medios/medios/${this.props.id_medio}/activo`]=false
      multiPath[`Servicios/Linkbuilding/Paid/Medios/medios/${this.props.id_medio}/eliminado`]=false
    }
  ...
  render() {
    ...
    return(
      <div className='sub-container-informacion'>
        ...
        {/*dr*/}
        <div className='col-2-input'>
          <SimpleInput type='int' title='DR' _class_container={this.state.dr.trim()==='' ? 'error-form-input':null}  text={this.state.dr} changeValue={dr=>{this.setState({dr})}}/>
          <SimpleInput type='int' title='UR' _class_container={this.state.ur.trim()==='' ? 'error-form-input':null}  text={this.state.ur} changeValue={ur=>{this.setState({ur})}}/>
        </div>
