...
class InformacionAdicional extends Component {
  ...
  saveData = () => {
    ...
    var multiPath = {}
    multiPath[`Clientes/${this.props.id_cliente}/servicios/linkbuilding/free/comentarios`] = this.state.comentarios.trim();
    ...
    if (this.props.comentarios !== this.state.comentarios.trim()) {
      ...
      functions.createLogs(multiPath, timestamp, this.props.comentarios, this.state.comentarios, 'comentarios', id_empleado, `Servicios/Logs/clientes/${this.props.id_cliente}/informacion/linkbuilding_gratuito/${id_log}`)
    }
  ...
  render() {
    ...
    try {
      privilegio = this.props.empleado.privilegios.linkbuilding_free.edit.info_adicional;
    } catch (e) { }
