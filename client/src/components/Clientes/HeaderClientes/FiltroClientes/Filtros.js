...
import { setFiltrosClientesLista, setPopUpInfo } from '../../../../redux/actions';
...
class Filtros extends Component {
  ...
  addNew = () => {
    try {
      ...
      this.props.setPopUpInfo({ visibility: true, text: 'No tienes suficientes permisos', status: 'close', moment: Date.now() })
      return null
    }
...
function mapStateToProps(state) { return { filtros_clientes_lista: state.filtros_clientes_lista, empleado: state.empleado } }
function matchDispatchToProps(dispatch) { return bindActionCreators({ setFiltrosClientesLista, setPopUpInfo }, dispatch) }
