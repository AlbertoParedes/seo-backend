...
class PanelDates extends Component {
  ...
  getData = () =>{
    if(this.state.keyword){
      db.child(`Resultados/${this.state.cliente.id_cliente}/${this.state.keyword.id_keyword}/dates`).on("value", snapshot =>{
        var resultados = {};
  ...
  render() {
    return (
      ...
      <div id='container-clientes-tracking-keywords-resultados' className={`container-table ${!this.props.visibility?'display_none':''}`} ref={scroller => {this.scroller = scroller}} onScroll={this.handleScroll}>
...
function mapStateToProps(state){return{cliente:state.cliente_seleccionado, keyword:state.keyword_tracking_selected}}
export default connect(mapStateToProps)(PanelDates);
