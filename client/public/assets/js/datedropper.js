(function($) {
	var
			...
			if(pickers[picker.id].lock) {
				...
				if(pickers[picker.id].lock=='to') {
					if(unix_current>unix_today) {
						picker_alrt();
						picker.element.addClass('picker-lkd');
						$(picker.element).find('i.lock_google').addClass('display_block');
						return true;
					}
					else {
						picker.element.removeClass('picker-lkd');
						$(picker.element).find('i.lock_google').removeClass('display_block');
						return false;
					}
				}
			}
		...
		picker_ul_transition = function(k,i) {
			...
			//ALBERTO------------------------------------------------------------------------------------------------------------------------------------
			//Añadiremos el mes y el año en nuestro imput como data-
			var id = $(ul).closest('div.datedropper').attr('id');
			var id_input = $(`input.picker-input[data-id="${id}"]`).attr('id');
			$(`input#${id_input}`).attr(`data-${k}-sld`,i);
		...
		input_change_value = function() {
			...
			if(!is_locked()&&picker_ctrl) {
				...
				//Alberto
				//clickamos el input para que guarde los cambios
				$(picker.input).click();
				picker_ctrl = false;
	...
	$.fn.dateDropper = function(options) {
		return $(this).each(function(){
			if($(this).is('input')&&!$(this).hasClass('picker-input')) {
				var id_alberto = $(this).attr('id')
				var existe = $('div[data-id-alberto="'+id_alberto+'"]').length
				if(existe>0){
					$('div[data-id-alberto="'+id_alberto+'"]').remove()
				}
				var
					input = $(this),
				...
				for( var k in pickers[id].key ) {
					var title = "";
					if(k==="m"){title="Mes"}
					else if(k==="y"){title="Año"}
					if(title!==""){
						$('<div>', {class: title !=="Mes" ? 'title_pop_up data_picker_titles space_title':'title_pop_up data_picker_titles', text: title}).appendTo(get_picker_els('.picker'));
					}
				...
				//Añadir candado Personalizando
				$('<i class="material-icons lock_google" > lock </i>', {class: ''}).appendTo(get_picker_els('.picker'));
