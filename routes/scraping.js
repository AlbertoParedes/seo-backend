  ...
  return new Promise( (resolve, reject) => {
    ...
    request.get({
      ...
      headers: {
        ...
        item.home = {}
        item.home.redirects = [];
        var canonical=null, robots=null;
        if(error){
    ...
    request.get({
      ...
      headers: {
        ...
        if(error){
          ...
          item.sitemaps.sitemap.redirects = response.request._redirect.redirects;
    ...
    request.get({
      ...
      headers: {
        ...
        if(error){
          ...
          item.sitemaps.sitemap_index.redirects = response.request._redirect.redirects;
    ...
    request.get({
      ...
      headers: {
        ...
        if(error){
          ...
          item.sitemaps['1_index_sitemap'].redirects = response.request._redirect.redirects;
...
router.get('/send-email-prueba', (req, res) => {

  var html = `<h1>Prueba Mail</h1>`
...
router.get('/run-status-task', (req, res) => {
  req.connection.setTimeout(20*60*1000)
  console.log('Start');
  var clientes = data, promesas=[], clientes_ordenados=[],n=0;
  Object.entries(clientes).forEach( ( [key, item] )=>{
    ...
    var promesa = new Promise( (resolve, reject) => {
      ...
      setTimeout(() => {
        ...
        item.sitemaps={ sitemap:{}, sitemap_index:{}, '1_index_sitemap':{} }
        var promesasItem = [];
...
function getHtml(clientes_ordenados){
  ...
  clientes_ordenados.map((c,k)=>{
    ...
    filas=filas + `
      ...
      <tr>
        <td class='st_col-status-status'><div class='st_status-point ${c.status===1?'st_good-status':''} ${c.status===2?'st_warning-status':''} ${c.status===3?'st_wrong-status':''}     ' ></div></td>

        <td class='st_col-status-web'>
          <a href='${c.web}' target='_blank'>${c.web}</a>
        </td>

        <td class='st_col-status-code ${c.home && (c.home.status>=400 || c.home.status==='Error') ?'st_wrong-status-color':''}' >${c.home? c.home.status : '-' }</td>

        <td class='st_col-status-meta ${c.home && c.home.robots && (c.home.robots.toLowerCase().includes('noindex') || c.home.robots.toLowerCase().includes('nofollow') )?'st_wrong-status-color':'' }'>
          <a href='${c.web}' target='_blank'>${c.home? c.home.robots : '-' }</a>
        </td>

        <td class='st_col-status-txt ${txt==='Disallow' || txt==='Error' || txt===404?'st_warning-status-color':''}'>
          <a href='${c.web+'/robots.txt'}' target='_blank'>${c.robots? txt : '-' }</a>
        </td>

        <td class='st_col-status-num ${num_sitemap_index===404?'st_warning-status-color':''}' >
          <a href='${c.web+'/sitemap_index.xml'}' target='_blank'>${c.sitemaps? num_sitemap_index : '-' }</a>
        </td>


        <td class='st_col-status-site ${sites==='Warning'?'st_warning-status-color':''}' >${sites}</td>
